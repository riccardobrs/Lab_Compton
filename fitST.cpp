/*
    ...
    Inoltre aggiorna il contenuto di un file "resolutionsST2.txt" da passare come argv[1] di "risol.cpp"
    NB: il file "resolutionsST2.txt" DEVE essere già esistente nella cartella, perché oltre ad essere aperto
...
//    ofstream outfile ("fitResult_"+fileInput);
    ...
    double ST;
    double ST_err = 0.1; // errore fissato "a mano"
    file_in = fileInput.replace(21, 4, ".png"); //il primo numero è la posizione dell'ultimo "." ---> modificare se necessario

    string fileInput2 = fileInput;
    fileInput.clear();
    for(int j=18; j<21; j++) {
        if(fileInput2[j]!=0)
            fileInput.push_back(fileInput2[j]);
    }
    ST = (double) stoi(fileInput) / 10;
...
/*
    Aggiungo ad un file "resolutionsST2.txt" delle righe così costruite:
    
    |ST|ST_err|Risoluz_picco1|Risoluz_picco1_err|Risoluz_picco2|Risoluz_picco2_err|
*/

    string resolutions = "resolutionsST2.txt";
    bool notwrite = false;
    ...
    if (out_in.good() == false) {
            cout << "File 'resolutionsST2.txt' inesistente o corrotto" << endl;
            return 1;
    }
    while(true) {
        out_in >> o1 >> o2 >> o3 >> o4 >> o5 >> o6;
        if(out_in.eof() == true) break;
        if(o1==ST) notwrite = true; //se è già presente non va scritto nuovamente
    }
    out_in.close();
    fstream out_app (resolutions.c_str(), ios::app); //apro il file in modalità append (scrittura: aggiungo righe alle preesistenti)
    if(notwrite == false)
        out_app << ST << "\t" << ST_err << "\t" << res1 << "\t" << res1_err << "\t" << res2 << "\t" << res2_err << endl;
    out_app.close();
/*
